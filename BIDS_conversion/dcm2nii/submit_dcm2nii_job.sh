...
# Paths come from config/pipeline.env
set -uo pipefail
...
#SBATCH --time=${DCM2NII_TIME}
#SBATCH --mem=${DCM2NII_MEM}
#SBATCH --cpus-per-task=${DCM2NII_CPUS}
...
bash "${stage_dir}/dcm2niix_func.sh" "\$pid" "\$session_date" "\$session_num" "$input_dir" "$output_dir"
EOF
