#!/bin/bash
# BIDS conversion -- behavioral data only.
...
symlink_behav_to_bids() {
    local behav_root="$1"     # Source behavioral folder ($BEHAV_DIR)
    local bids_root="$2"      # Target imaging BIDS folder ($BIDS_DIR)
    local subj="$3"           # Subject ID, without the "sub-" prefix
