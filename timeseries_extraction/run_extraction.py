...
def resample_roi(roi_img, target_img):
    """Put an ROI mask onto the functional image's voxel grid.

    KNOWN LIMITATION -- this is an affine grid resampling only. It changes
    voxel size and grid alignment; it applies NO nonlinear transform. That is
    correct only when the ROI mask is already defined in the same template
    space as the functional data (here MNI152NLin2009cAsym).

    An ROI defined in any other space needs the nonlinear warp applied with
    ANTs, e.g. antsApplyTransforms driven by the transform fMRIPrep emits:

        *_from-MNI152NLin2009cAsym_to-T1w_mode-image_xfm.h5

    Resampling alone in that case silently produces misaligned ROIs and
    therefore invalid timeseries -- it will not raise. See README, "Status and
    known limitations".
    """
...
def main(args):
    ...
    for run, direction in run_dir_pairs:
        stem = ("sub-"+subid+"_ses-"+sesid+"_task-"+task_label
                +"_dir-"+direction+"_run-"+run)
        func_dir = os.path.join("sub-"+subid, "ses-"+sesid, "func")

        fmri_file = os.path.join(fmriprep_base, func_dir,
            stem+"_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz")
        confound_file = os.path.join(fmriprep_base, func_dir,
            stem+"_desc-confounds_timeseries.tsv")
        events_file = os.path.join(events_base, func_dir,
            stem+"_events.tsv")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_label", default="WM",
                        help="BIDS task-<label> to extract (default: WM)")
